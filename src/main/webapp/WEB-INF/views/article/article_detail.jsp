...
<html class="no-js">
	...
	<title>${article.title}&nbsp;- CODER 博客's 文章</title>
    ...
    <div class="container">
		...
		<article class="col-md-9 col-sm-12 col-xs-12" id="main" role="main">
			...
			<header class="post post-container article_header" style="overflow:visible">
				...
				<p>
					<ul class="post-meta">
						<li><time class="article_time" title="更新时间：<fmt:formatDate value='${article.update_time}' pattern='yyyy-MM-dd HH:mm'/>" datetime="${article.create_time}" itemprop="datePublished"><fmt:formatDate value="${article.create_time}" pattern="yyyy-MM-dd HH:mm"/></time></li>
					...
					<div class="btn-group	article_handle" style="margin-top:-33px;float:right">
						...
						<ul class="dropdown-menu" style="min-width:30px">
							...
							<c:if test="${ not empty loginUser and loginUser.uid == article.author.uid }">
								...
								<li><a style="padding:4px 12px 3px 12px;" id="showDeleteModalBtn" onclick="$('#validateMailModal').modal({backdrop: 'static', keyboard: false});" ><span class="glyphicon glyphicon-trash" aria-hidden="true"></span><b> 删除</b></a></li>
							</c:if>
			...
			<article>
				...
				<div id="comments-respond-post" class="respond">
					...
					<form method="post" action="" id="comment-form" role="form" class="card">
						...
						<p>
							<button type="button" id="comment_form_submit" class="submit ripple"><span>提交评论</span><ripple-container></ripple-container></button>
						</p>
					</form>
				</div>
				<!-- 评论 input end -->
