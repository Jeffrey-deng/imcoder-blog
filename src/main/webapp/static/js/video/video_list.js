...
(function (factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        define(['jquery', 'bootstrap', 'domReady', 'toastr', 'globals', 'common_utils', 'login_handle', 'period_cache', 'video_handle', 'album_photo_page_handle', 'album_video_plugin'], factory);
    } else {
        // Browser globals
        factory(window.jQuery, null, $(document).ready, toastr, globals, common_utils, login_handle, PeriodCache, video_handle, album_photo_page_handle, album_video_plugin);
    }
})(function ($, bootstrap, domReady, toastr, globals, common_utils, login_handle, PeriodCache, video_handle, album_photo_page_handle, album_video_plugin) {
    ...
    const request = globals.extend(globals.request, {
        video_list: {
            'loadVideoList': function (condition, success) {
                return globals.request.get(globals.api.getVideoList, condition, success, ['videos'], success && '加载视频列表失败');
            },
                    ...
                    setTimeout(function () {
                        $.each(videoList, function (i, video) {
                            if (video.cover.photo_id == cover_id) {
                                ...
                                applyArgs = [video];
                                callback && callback.apply(response, applyArgs);
                                dfd.resolveWith(response, applyArgs);
                                return false;
                            }
                        });
                    }, 0);
            ...
            'loadVideosByCoversOverride': function (array, success) {
                return $.Deferred(function (dfd) {
                    setTimeout(function () {    // 这里要异步因为checkPhotoId，不异步的阻断函数反而阻断自己的click，使popup失效
                        let videos = [], response, applyArgs;
                        for (let i in array) {
                            let cover_id = array[i];
                            for (let j in videoList) {
                        ...
                        applyArgs = [videos];
                        success && success.apply(response, applyArgs);
                        dfd.resolveWith(response, applyArgs);
                    }, 0);
                });
            }
    ...
    domReady(function () {
        ...
        if (clearPageMatch) {
            if (RegExp.$2 == 'topic') {
                ...
                title_suffix = $firstArea.find('.slogan-name').text() + "喜欢的视频 | ImCoder博客's 相册";
                ...
                title_suffix = $firstArea.find('.slogan-name').text() + "访问过的视频 | ImCoder博客's 相册";
                !load_condition.from && (load_condition.from = "user_history_videos");
            } else {
                isClearUserPage = true;
                load_condition["user.uid"] = clearPageMatch[7];
                title_suffix = $firstArea.find('.slogan-name').text() + "的视频 | ImCoder博客's 相册";
        ...
        album_photo_page_handle.init({
            ...
            callback: {
                "requestPhotoList": function (condition, call) {
                    let context = this;
                    globals.notify({}).progress('正在加载数据', '', 'notify_videos_loading');
                    return request.loadVideoList(condition, true).always(function () {
                        globals.removeNotify('notify_videos_loading');
                    }).final(function (videos) {
                        let data = this.data;
                        if (!videoList || videoList.length == 0) {
                            videoList = videos;
                        } else {
                            videoList.push.apply(videoList, videos);
                        }
                        data.photos = convertVideoListToPhotoList(videos);
        ...
        video_handle.init({
            ...
            "callback": {
                ...
                "updateCompleted": function (context, video) {  // 更新完成后回调
                    let index = getVideoIndexInList(video);
                ...
                "beforeUploadModalOpen": function (context, uploadModal, openUploadModal_callback) {  // 上传窗口打开前回调
                    // 加载相册列表
                    let hostUser = context.config.hostUser;
                    let dfd = $.Deferred();
                    ...
                    dfd.done(function () {
                        ...
                        $.get(globals.api.getVideoUploadConfigInfo, function (response) {
                            if (response.status == 200) {
                                ...
                                if (!uploadConfigInfo || uploadConfigInfo.isAllowUpload) {
                                    ...
                                    globals.notify({timeOut: 0}).info('系统当前配置为只允许<br>【<b>' + users + '</b>】上传视频', '您暂时不能上传', 'notify-no-allow-upload');
                                    // 禁用上传按钮
                ...
                "beforeUpdateModalOpen": function (context, updateModal, formatVideoToModal_callback, video) {  // 更新窗口打开前回调
                    ...
                    if (typeof video == 'object' && video.video_id) {
                        dfd.resolve(video);
                    } else if (typeof video == 'object' && video.cover_id) {
                        request.loadVideoByCoverOverride(video.cover_id, function (video) {
                            dfd.resolve(video);
                        });
                    } else if (typeof video == 'string') {  // 如果传入的参数为video_id，获取video对象
                        let videoIndex = getVideoIndexInList({"video_id": video});
                        if (videoIndex != -1) {
                            dfd.resolve(videoList[videoIndex]);
                        } else {
                            dfd.reject();
                        }
                    } else {
        ...
        video_handle.on(video_handle.config.event.tagClick, function (_e, tag, video_id, clickEvt) {
            ...
            window.open(('p/tag/' + tag + (isClearUserPage ? ('?uid=' + load_condition['user.uid']) : '')).toURL());
        });
        ...
        // 绑定视频插件中编辑按钮事件
        album_video_plugin.on(album_video_plugin.config.event.actionForEditVideo, function (e, video) {
        ...
        if (isClearUserHistoryPage || isClearUserLikesPage) {
            ...
            $deleteAccessRecordBtn.on('click', function () {
                let video_id = video_handle.pointer.updateModal.find('span[name="video_id"]').html().trim();
                if (isClearUserHistoryPage) {
                    globals.request.post(globals.api.deleteUserVideoAccessDetail, {"video_id": video_id}, function () {
                        toastr.success('已删除此访问记录~');
                        video_handle.pointer.updateModal.modal('hide');
                    });
                } else {
                    globals.request.post(globals.api.likeVideo, {"video_id": video_id, "undo": true}, function () {
                        toastr.success('已取消赞~');
                        video_handle.pointer.updateModal.modal('hide');
                    });
                }
        ...
        $('.video_options .option_run_tags_classification').url('href', tags_classification_href).show();
