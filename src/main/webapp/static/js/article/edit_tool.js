...
//                   '        </span>' +
    ...
    function sendFile(files, index) {
        ...
        var notify_uploading = toastr.info("正在上传图片" + ( (index === 0 && files.length === 1) ? "" : "第" + (index + 1) + "张" ), "提示", {"progressBar": false, "hideDuration": 0, "timeOut": 0, "closeButton": false});
        $.ajax({
            data: data,
            type: "POST",
    ...
    function localImage($image) {
        ...
        var notify_downloading = toastr.info("服务器正在下载图片", "提示", {"progressBar": false, "hideDuration": 0, "timeOut": 0, "closeButton": false});
        $.ajax({
