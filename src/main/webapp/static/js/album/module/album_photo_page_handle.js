...
(function (factory) {
                ...
                $('#' + config.selector.photosContainer_id)
                    .on({
                        ...
                        "dragend": function (e) {
                            ...
                            context.trigger(config.event.actionForEditPhoto, photo);
                        }
                    }, "img")
    ...
    var request = {
        "triggerPhotoAccess": function (photo_id, callback) {
            if (!photo_id) {
                return;
            }
            var post_param = null;
            if (typeof photo_id == "object") {
                post_param = {"photo_id": photo_id.photo_id};
            } else {
            ...
            $.post("photo.api?method=triggerPhotoAccess", post_param, function (response) {
                if (response.status == 200) {
                    callback && callback(response.data.photo);
                }
            });
        },
    ...
    var utils = {
        "createPhotoNode": function (photo) {
            ...
            if (config.photoNodeLinkUsePhotoDetail) {
                a.href = 'p/detail/' + photo.photo_id;
            } else {
                a.href = photo.path;
            }
        ...
        $("#" + config.selector.photosContainer_id).magnificPopup({
            ...
            callbacks: {
                ...
                markupParse: function (template, values, item) {
                    ...
                    var photoDetailLink = 'p/detail/' + photo_id;
                    if (config.photoNodeLinkUsePhotoDetail) {
                        template.find('.photo-detail-link').attr('href', photoDetailLink);
                    } else {
                        template.find('.photo-detail-link').attr('href', photo.path);
                    }
                    var trigger = '<a style="margin-right: 7px;" class="btn-like-photo' + (liked ? ' photo-has-liked' : '') + '" ' +
                        'data-photo-id=' + photo_id + ' title="添加到喜欢" href="u/likes/photos" target="_blank"' + '>' + (liked ? '已赞' : '赞') + '</a>' +
                        '<a class="btn-open-photo-detail" ' +
                        'data-photo-id=' + photo_id + ' title="点击打开图片详情页" href="' + photoDetailLink + '#comments" target="_blank"' + '>评论</a>';
                    if (isAuthor) {
                        trigger += '<a class="btn-open-update-modal" data-photo-id=' + photo_id + ' href="' + photoDetailLink + '?mark=edit" target="_blank" title="点击编辑图片信息">编辑</a>'
                    } else {
                        trigger += '<a class="btn-open-update-modal" data-photo-id=' + photo_id + ' href="' + photoDetailLink + '?mark=meta" target="_blank" title="点击查看图片信息">属性</a>'
                    }
                ...
                change: function () {
                    ...
                    mfp.content.on('click', '.btn-open-update-modal', function (e) {
                        ...
                        request.likePhoto(photo.photo_id, undo, function (newPhoto, type, response) {
                            var newValue = newPhoto.like_count;
                            if (type == 1) {
                                if (undo) {
                                    toastr.success("已移除赞~");
                                } else {
            ...
            image: {
                ...
                titleSrc: function (item) {
                    ...
                    var photoDetailUrl = "p/detail/" + photo.photo_id;
                    if ((!name) && desc) {
                        name = desc;
                        desc = "";
                    }
                    return '<a href="' + photoDetailUrl + '" target="_blank" title="点击新标签打开图片">' + common_utils.encodeHTML(name) + '</a>' +
                        '<small>' + common_utils.encodeHTML(desc) + '</small>';
                },
