...
public class ConfigManager {
    ...
    private void initServerConfig(ServletContext servletContext) {
        ...
        if (serverConfigLocation != null) {
            serverConfigLocation = serverConfigLocation.trim();
            if (serverConfigLocation.startsWith("/")) {
                serverConfigLocation = serverConfigLocation.substring(1);
            }
    ...
    private void initLogFilePath(ServletContext servletContext) {
        ...
        if (default_info_path == null || default_warn_path == null || default_error_path == null) {
            ...
            if (path != null) {
                ...
                this.log4jConfigPath = path;
                Properties properties = new Properties();
                BufferedReader bufferedReader = null;
                try {
                    bufferedReader = new BufferedReader(new FileReader(Utils.getContextRealPath() + path));
                    ...
                    e.printStackTrace();
                    logger.error("日志文件流读取出错, 文件未找到: " + Utils.getContextRealPath() + path, e);
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    private void loadConfig() {
        Config.setDefault();
        loadConfigFromXML(serverConfigLocation, "init");
    ...
    public void reloadConfig() {
        loadConfigFromXML(serverConfigLocation, "update");
    }
    ...
    public void loadConfigFromXML(String configPath, String type) {
        ...
        String path = Utils.getContextRealPath() + configPath;
