...
public class ImageUtil {
	...
	public static int[]  getWH(File file) throws IOException {
		SimpleImageInfo imageInfo = new SimpleImageInfo(file);
		return new int[]{imageInfo.getWidth(),imageInfo.getHeight()};
	}

	public static SimpleImageInfo  getImageInfo(File file) throws IOException {
		SimpleImageInfo imageInfo = new SimpleImageInfo(file);
		return imageInfo;
	}
	...
	public static class SimpleImageInfo {
		...
		public SimpleImageInfo(File file) throws IOException {
			...
			try {
				processStream(is,file);
			} finally {
				is.close();
			}
		}

		private void processStream(InputStream is, File file) throws IOException {
			int c1 = is.read();
			int c2 = is.read();
			int c3 = is.read();
			...
			if (mimeType == null) {
				...
				try {
					...
					in.read(arr);
					if ("WEBP".equals(new String(arr, 8, 4, "utf-8"))) { // webp
						width = (((int) arr[27] & 0xff) << 8) | (int) arr[26] & 0xff;
						height = (((int) arr[29] & 0xff) << 8) | (int) arr[28] & 0xff;
						mimeType = "image/webp";
					} else {
